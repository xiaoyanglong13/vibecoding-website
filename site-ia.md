...
> ~320 min, with `course/index.html`'s lede and the home page's hours card
> bumped to match), then removed again later the same day at the user's
> request, along with its parallel `/cases/operations/demand-forecast-dashboard`
> copy, that copy's sample-solution artifact, and all three sample-data CSVs
> — see the Module 7 section and §9 for the cases-library side of the
> removal. The module is back to 8 pages / ~200 min, every "of 9" reverted to
> "of 8", use-case-model-automation's closing CTA points at Module 5 again,
...
- **use-case-model-automation** — originally the same treatment as
  ...
  used) unchanged. Ends with the module's closing CTA to Module 5 (briefly
  repointed to a page 9 while use-case-demand-forecast existed — see below —
  reverted when that page was removed). Runs second as the longer, more
  procedural case.
- **use-case-demand-forecast** — existed only from 2026-08-27 to later the
  same day. It was added as page 9 of 9, a retail-demand-forecast-plus-
  dashboard analytics case sharing sample data with a parallel
  `/cases/operations/demand-forecast-dashboard` copy (plus that copy's
  generated `demand-forecast-sample-solution.html` artifact and three sample
  CSVs), then removed entirely at the user's request the same day — see the
  dated note at the top of this section. No trace of either copy remains;
  don't recreate one from this note without a fresh request.
...
1. One additional case per function category to demonstrate breadth (Operations,
   Sales, and Founder are all still index-only).
2. Module 5 (Scaling up) sub-pages — version control (Git/GitHub), extending
